...
async def get_model_with_fallbacks(db, model_id):
    ...
    """
    try:
        model = await sync_to_async(db.aimodel.find_unique)(
            where={"id": model_id},
        ...
        for fallback_key in ['fallback1Id', 'fallback2Id', 'fallback3Id']:
            ...
            if fallback_id:
                fb_model = await sync_to_async(db.aimodel.find_unique)(
...
async def check_orchestrator_model(db, model_id):
    ...
    try:
        model = await sync_to_async(db.aimodel.find_unique)(where={"id": model_id})
...
async def execute_orchestrator_request(request, db, messages, model_data, selected_functions=None, output_schema=None, compiled_rules=None):
    ...
    if is_orch_client:
        ...
        orch_funcs = await sync_to_async(db.orchfunction.find_many)(
            where={"enabled": True, "name": {"in": list(selected_functions)}}
        )
        
        if not orch_funcs or len(orch_funcs) == 0:
            all_enabled = await sync_to_async(db.orchfunction.find_many)(where={"enabled": True})
        ...
        user_funcs = await sync_to_async(db.userfunction.find_many)(
        ...
        if not user_functions or len(user_functions) == 0:
            all_user_funcs = await sync_to_async(db.userfunction.find_many)(where={"userId": user_id, "enabled": True})
    ...
    for uf in user_functions:
        if isinstance(uf, dict):
            ...
            if is_orch_client:
                ...
                keys = await sync_to_async(db.userfunctionkey.find_many)(
    ...
    # Log execution
    await sync_to_async(db.orchexecution.create)(
        data={
            "id": str(uuid.uuid4()),
    ...
    if output_schema and results:
        structured = await format_output_with_schema(
...
{{"chave1": "valor_ou_objeto", "chave2": "valor_ou_objeto"}}"""
    ...
    model_info = await sync_to_async(db.aimodel.find_unique)(
...
async def format_output_with_schema(db, model_data, extraction_results, output_schema, user_content, system_prompt="", compiled_rules=None):
    """
